...
- **Volume / VolumeMount Order (Kubernetes Note)**:
  - **Volumes (`volumes:`)** are created or attached **before any container starts**.  
    This includes PVCs, `emptyDir`, and ConfigMaps.
  - **InitContainers** run **after volumes exist** and can read/write them.
  - **Main containers** run **after all initContainers complete** and see whatever initContainers wrote to the volumes.
  - **VolumeMounts** are per-container mappings that specify where a volume appears inside that container.
  - This separation explains why Kubernetes requires both `volumes:` (definition) and `volumeMounts:` (container path).  
    InitContainers can prepare data or certs in volumes so main containers start with everything ready.

...
// // returns 42
//
// _p = 99; // "go to address 0x7fff1234, write 99 there"
// // x is now 99
//
// The _ operator means "follow this address and read/write what's there".
...
**Why loopback only?**
The default bind address is `127.0.0.1`, not `0.0.0.0`. Nothing on the network can reach it. You _can_ override with `--address=0.0.0.0`, but that exposes the port with no authentication. The SSH tunnel is the right way to bring it to your laptop.

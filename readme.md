...
- Inline documentation: architecture decisions are explained in the README but not always at the point where they are implemented. Currently going through every manifest and script to add inline comments explaining why each config value exists, what breaks if you change it, and what alternatives were considered and rejected. The goal is that any file can be read in isolation without needing to cross-reference the README.
